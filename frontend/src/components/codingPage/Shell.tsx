...
export const Shell: React.FC<ShellProps> = ({ socket, onClose }) => {
  ...
  useEffect(() => {
    console.log('[Shell] Socket effect triggered - socket:', !!socket, 'isTerminalReady:', isTerminalReady);
  ...
  // Handle initial socket connection state
  useEffect(() => {
    ...
    if (socket && socket.connected && !isConnected) {
      console.log('[Shell] Socket already connected on mount, setting connection state');
      setIsConnected(true);
      
      // Start terminal session if terminal is ready
      if (isTerminalReady) {
        const replId = `repl_${Date.now()}`;
        setTerminalId(replId);
        console.log('[Shell] Starting terminal session with replId:', replId);
  ...
  return (
    <div className="h-full w-full flex flex-col bg-gray-900 overflow-hidden">
      ...
      <div 
        ref={terminalRef} 
        className="flex-1 p-2 cursor-text w-[100%] overflow-hidden"
        style={{ 
