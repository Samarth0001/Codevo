...
const ForgotPasswordPage = () => {
  ...
  const validateToken = async () => {
    try {
      ...
      // For forgot password, we'll use a different validation endpoint
      // Since we don't have a specific forgot password validation endpoint,
      // we'll try to validate it by attempting to use it
      setTokenValid(true);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use the dedicated forgot password reset API
      const response = await PasswordResetAPI.resetPasswordWithToken(token, formData.newPassword);
      ...
      toast.error(error.response?.data?.message || "Failed to reset password. The link may have expired or is invalid.");
    } finally {
      setIsLoading(false);
    }
  };
