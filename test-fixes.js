...
// Test configuration
const TEST_PROJECT_ID = 'test-fixes-123';
...
async function testFixes() {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      // Also listen for manual file structure response
      socket.on('files:structure', (structure) => {
        console.log('✅ Received manual file structure:', structure.length, 'items');
        resolve();
      });

      setTimeout(() => {
        console.log('⚠️  Project initialization timeout, trying manual file structure request');
